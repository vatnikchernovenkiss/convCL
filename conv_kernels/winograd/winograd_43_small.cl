...
kernel void winograd(const __global float *data, const __global float *filters, const int H, const int W,
            ...
            for (int i = 0; i < transformed_buf_H; ++ i) {
                for (int j = 0; j < transformed_buf_W; ++j) {
                  transformed_matrix_buf[i * transformed_buf_W + j] = filters[filter_offset + c * 
                                         transformed_buf_size + i * transformed_buf_W + j];
                }
            }
        }
        ...
        for (int k = 0; k < channel_num; ++k) {
            for (int el = 0; el < ACCUM_BUF_SIZE_IMG; ++el) {
               ...
               accum_img[el] = local_buf[(img_local_id / (img_group_size / transformed_buf_size)) * 
                                         CHANNELS_PER_GROUP * TILES_PER_GROUP +
                                         k * TILES_PER_GROUP + ((img_local_id % THREADS_PER_ONE_BUF_ELEM) / 
                                                                 N_ITER_PER_ONE_BUF_ELEM_IMG) *
                                         ACCUM_BUF_SIZE_IMG + el];
            }
            for (int el = 0; el < ACCUM_BUF_SIZE_FILTER; ++el) {

               accum_kern[el] = local_buf[local_img_buf_size + (filter_local_id /
                                                                (img_group_size / transformed_buf_size)) * 
                                          CHANNELS_PER_GROUP * TILES_PER_GROUP + k * TILES_PER_GROUP + 
                                          ((filter_local_id % THREADS_PER_ONE_BUF_ELEM) %                                                                  N_ITER_PER_ONE_BUF_ELEM_FILTER) +
    ...
    for (int filter_count = 0, accum_buf_beg=0; filter_count < TILES_PER_GROUP; 
        ...
        for (int i = img_local_id; i < filters_in_buf * TILES_PER_GROUP; i += TILES_PER_GROUP * CHANNELS_PER_GROUP) {
            ...
            matrix_buf[5] = transformed_matrix_buf[30] + transformed_matrix_buf[31] + 
                            ...
                            transformed_matrix_buf[34];
            matrix_buf[6] = transformed_matrix_buf[1] - transformed_matrix_buf[2] + 
                            2 * (transformed_matrix_buf[3] - transformed_matrix_buf[4]);
            matrix_buf[7] = transformed_matrix_buf[7] - transformed_matrix_buf[8] + 
            ...
            matrix_buf[12] = transformed_matrix_buf[1] + transformed_matrix_buf[2] + 
                             4 * (transformed_matrix_buf[3] + transformed_matrix_buf[4]);
            matrix_buf[13] = transformed_matrix_buf[7] + transformed_matrix_buf[8] +
                             4 * (transformed_matrix_buf[9] + transformed_matrix_buf[10]);
            matrix_buf[14] = transformed_matrix_buf[13] + transformed_matrix_buf[14] +
                             4 * (transformed_matrix_buf[15] + transformed_matrix_buf[16]);
            matrix_buf[15] = transformed_matrix_buf[19] + transformed_matrix_buf[20] +
                             4 * (transformed_matrix_buf[21] + transformed_matrix_buf[22]);
            matrix_buf[16] = transformed_matrix_buf[25] + transformed_matrix_buf[26] +
                             4 * (transformed_matrix_buf[27] + transformed_matrix_buf[28]);
            matrix_buf[17] = transformed_matrix_buf[31] + transformed_matrix_buf[32] +
                             4 * (transformed_matrix_buf[33] + transformed_matrix_buf[34]);
            matrix_buf[18] = transformed_matrix_buf[1] - transformed_matrix_buf[2] +
                             transformed_matrix_buf[5] + 8 * (transformed_matrix_buf[3] - 
                                                              transformed_matrix_buf[4]);
            matrix_buf[19] = transformed_matrix_buf[7] - transformed_matrix_buf[8] + transformed_matrix_buf[11]
                             + 8 * (transformed_matrix_buf[9] - transformed_matrix_buf[10]) ;
            matrix_buf[20] = transformed_matrix_buf[13] - transformed_matrix_buf[14] + 
                             transformed_matrix_buf[17] + 8 * (transformed_matrix_buf[15] - 
                                                               transformed_matrix_buf[16]);
            matrix_buf[21] = transformed_matrix_buf[19] - transformed_matrix_buf[20] + 
                             transformed_matrix_buf[23] + 8 * (transformed_matrix_buf[21] - 
                                                               transformed_matrix_buf[22]);
            matrix_buf[22] = transformed_matrix_buf[25] - transformed_matrix_buf[26] + 
                             transformed_matrix_buf[29] + 8 * (transformed_matrix_buf[27] - 
                                                               transformed_matrix_buf[28]);
            matrix_buf[23] = transformed_matrix_buf[31] - transformed_matrix_buf[32] + 
                             transformed_matrix_buf[35] + 8 * (transformed_matrix_buf[33] - 
                                                               transformed_matrix_buf[34]);
    
            const int output_filter_num =  i / TILES_PER_GROUP + filter_count, output_tile_num = 
                                                                               i % TILES_PER_GROUP;
            ...
            if ((output_row < H) && (img_num < N_objects) && (output_filter < N_filters)) {
                ...
                output[output_offset] = matrix_buf[0] + matrix_buf[1] + matrix_buf[2] + matrix_buf[3] + 
                                        matrix_buf[4];
                output[output_offset + 1] = matrix_buf[1] - matrix_buf[2] + 2 * (matrix_buf[3] - matrix_buf[4]);
                output[output_offset + 2] = matrix_buf[1] + matrix_buf[2] + 4 * (matrix_buf[3] + matrix_buf[4]);
                output[output_offset + 3] = matrix_buf[1] - matrix_buf[2] + matrix_buf[5] + 
                                           8 * (matrix_buf[3] - matrix_buf[4]);
    
                output[output_offset + W] = matrix_buf[6] + matrix_buf[7] + matrix_buf[8] + matrix_buf[9] + 
                                            matrix_buf[10];
                output[output_offset + W + 1] = matrix_buf[7] - matrix_buf[8] + 
                                                2 * (matrix_buf[9] - matrix_buf[10]);
                output[output_offset + W + 2] = matrix_buf[7] + matrix_buf[8] +
                                                4 * (matrix_buf[9] + matrix_buf[10]);
                output[output_offset + W + 3] = matrix_buf[7] - matrix_buf[8] + matrix_buf[11] +
                                                8 * (matrix_buf[9] - matrix_buf[10]);
    
                output[output_offset + 2 * W] = matrix_buf[12] + matrix_buf[13] + matrix_buf[14] + 
                                                matrix_buf[15] + matrix_buf[16];
                output[output_offset + 2 * W + 1] = matrix_buf[13] - matrix_buf[14] + 
                                                    2 * (matrix_buf[15] - matrix_buf[16]);
                output[output_offset + 2 * W + 2] = matrix_buf[13] + matrix_buf[14] + 
                                                    4 * (matrix_buf[15] + matrix_buf[16]);
                output[output_offset + 2 * W + 3] = matrix_buf[13] - matrix_buf[14] + matrix_buf[17] +
                                                    8 * (matrix_buf[15] - matrix_buf[16]);
    
                output[output_offset + 3 * W] = matrix_buf[18] + matrix_buf[19] + matrix_buf[20] + 
                                                matrix_buf[21] + matrix_buf[22];
                output[output_offset + 3 * W + 1] = matrix_buf[19] - matrix_buf[20] +
                                                    2 * (matrix_buf[21] - matrix_buf[22]);
                output[output_offset + 3 * W + 2] = matrix_buf[19] + matrix_buf[20] +
                                                    4 * (matrix_buf[21] + matrix_buf[22]);
                output[output_offset + 3 * W + 3] = matrix_buf[19] - matrix_buf[20] + matrix_buf[23] +
                                                    8 * (matrix_buf[21] - matrix_buf[22]);
            }
            barrier(CLK_LOCAL_MEM_FENCE);
        }
    }
