...
#define TILES_PER_GROUP 32
#define CHANNELS_PER_GROUP 8

#define ACCUM_BUF_SIZE_IMG 8
#define ACCUM_BUF_SIZE_FILTER 8

#define N_ITER_PER_ONE_BUF_ELEM_IMG  4
#define N_ITER_PER_ONE_BUF_ELEM_FILTER  4

#define THREADS_PER_ONE_BUF_ELEM 16
...
kernel void winograd(const __global float *data, const __global float *filters, const int H,const int W,
    ...
    const int transformed_buf_size = transformed_buf_H * transformed_buf_W;
    ...
    const int local_buf_size = 2 * CHANNELS_PER_GROUP * TILES_PER_GROUP * transformed_buf_size;
    ...
    for (int c = 0; c < N_channels; c += CHANNELS_PER_GROUP) {
        ...
        if ((row_to_process_init < H) && (col_to_process_init < W) && (img_num < N_objects) &&
            ...
            for (int i = 0; i < img_buf_size; ++i) {
                cur_row = row_to_process_init + i / img_buf_W;
                cur_col = col_to_process_init + i % img_buf_W;
                
                if ((cur_row >= 0) && (cur_row < H) && (cur_col >= 0) && (cur_col < W)) {
                    img_buf[i] =
                    data[img_offset_c + c * img_size + (row_to_process_init + (i / img_buf_W)) * W +
                         col_to_process_init + i % img_buf_W];
                } else {
                    img_buf[i] = 0;
                }
            }
        ...
        if ((filter_tile_offset < N_filters) && ((img_channel_begin + c) < N_channels)) {
            ...
            for (int i = 0; i < transformed_buf_size; ++ i) {
                  filter_buf[i] = filters[filter_offset + c * transformed_buf_size + i];
            }
        }
            ...
            for (int el = 0; el < ACCUM_BUF_SIZE_FILTER; ++el) {
               ...
               accum_kern[el] = local_buf[local_img_buf_size + (filter_local_id /
                                          ...
                                          ((filter_local_id % THREADS_PER_ONE_BUF_ELEM) % 
                                           N_ITER_PER_ONE_BUF_ELEM_FILTER) + el *
    ...
    for (int filter_count = 0, accum_buf_beg = 0; filter_count < TILES_PER_GROUP;
        ...
        float transform_accum_buf[transformed_buf_size];
        float tmp[8];
